from models import app, Customer, db, Item, Purchase, Purchase_detail
...
def select_item_name():
    input_item_name = request.form["input-item-name"]
    select_target = "%{}%".format(input_item_name)
...
def add_purchase():
    ...
    item_quantity1 = request.form["input-quantity1"]
    item_name2 = request.form["input-item-name2"]
    item_quantity2 = request.form["input-quantity2"]
    ...
    purchase_detail = Purchase_detail(purchase.purchase_id, item1.item_id, item_quantity1) 
    try:
    ...
    if item_name2:
        ...
        purchase_detail = Purchase_detail(purchase.purchase_id, item2.item_id, item_quantity2) 
        try:
            db.session.add(purchase_detail)
            db.session.commit()
        except sqlalchemy.exc.IntegrityError:
...
def search_purchase():
    ...
    if is_customer_or_date and item_name:
        purchase_details = Purchase_detail.query.filter(Purchase_detail.item_id.in_(item_id_list), Purchase_detail.purchase_id.in_(purchase_id_list))
        return render_template("4-1_result_search_parchase.html", purchase_details=purchase_details)
    elif is_customer_or_date:
        purchase_details = Purchase_detail.query.filter(Purchase_detail.purchase_id.in_(purchase_id_list))    
        return render_template("4-1_result_search_parchase.html", purchase_details=purchase_details)    
    elif item_name:
        purchase_details = Purchase_detail.query.filter(Purchase_detail.item_id.in_(item_id_list))
        return render_template("4-1_result_search_parchase.html", purchase_details=purchase_details)
    else:
        return render_template("error.html")
...
def ranking_items():
    ...
    def fanc(x):
        return x[1]
    #items()→listの中にタプルでkeyとvalueが入っているもの
    ...
    return render_template("4-3_result_ranking.html",statistics_type=statistics_type, items=items)    
